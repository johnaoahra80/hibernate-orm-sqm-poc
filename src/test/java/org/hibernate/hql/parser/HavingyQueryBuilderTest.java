 ...
 */
public class HavingyQueryBuilderTest {
	@Test
	public void simpleGroupByTest() {
		final ParsingContextTestingImpl parsingContext = new ParsingContextTestingImpl();

		final HqlParser parser = HqlParseTreeBuilder.INSTANCE.parseHql( "select cat.color, sum(cat.weight), count(cat) from Cat cat group by cat.color having cat.color='ginger'" );
		...
		assertNotNull( querySpec.getHavingClause() );

		assertNotNull( querySpec.getHavingClause().getPredicate() );
	}
